...
class Tokenizer:
    pattern: re.Pattern
    skip = r"([\s]*(;.*\n)?)*"
    opecode = r"add|sub|mul|div|cmp|abs|adc|sbc|shl|shr|ash|rol|ror|and|or|not|xor|setl|seth|load|store|jmpa|jmp|nop|hlt"
    register = r"r1?[0-9]"
    immdval = r"#[0-9]+"
    ...
    def tokenize(self, input: str) -> Iterator[Token]:
        ...
        while result := self.pattern.match(input, i):
            match result.groups()[-5:]:
                ...
                case (None, None, imm, None, None): yield Immdval(int(imm[1:]))
